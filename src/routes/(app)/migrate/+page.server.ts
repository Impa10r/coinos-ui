...
async function invoiceFor(address: string, sats: number) {
  ...
  if (!p?.callback)
    throw new Error(
      p?.reason || "that address isn't registered on coinos v3 yet",
    );
  const msat = sats * 1000;
  if (msat < p.minSendable)
    throw new Error("balance is below the destination's minimum");
  if (msat > p.maxSendable)
    throw new Error("balance is above the destination's maximum");
  const sep = p.callback.includes("?") ? "&" : "?";
  const inv = await fetch(`${p.callback}${sep}amount=${msat}`).then((r) =>
    r.json(),
  );
  ...
  default: async ({ cookies, request, url }) => {
    ...
    try {
      if (balance > 0) {
        ...
        const attempts = [
          balance,
          balance - Math.max(1, Math.ceil(balance * 0.005)),
        ];
        let lastErr;
