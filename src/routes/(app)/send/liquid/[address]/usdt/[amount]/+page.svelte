<script>
  ...
  $effect(() => {
    ...
    if (fixedSats) {
      ...
      currentUsdtAmount = parseFloat(((fixedSats / sats) * effectiveRate).toFixed(8));
    } else {
      currentUsdtAmount = usdtAmount;
      currentSats = Math.round((usdtAmount / effectiveRate) * sats);
    }
    currentFee = Math.round(currentSats * 0.001);
  });
...
<div class="container px-4 max-w-lg mx-auto space-y-8 text-2xl">
  ...
  <div class="btn w-full text-lg">
    1 <span style="color:#F7931A" class="font-bold">₿</span> = <span style="color:#26A17B">₮</span> {liveUsdtRate ? Math.round(liveUsdtRate / 1.0015).toLocaleString(locale) : "…"}
  </div>
  ...
  <div class="space-y-5">
    <div>
      <div class="text-lg text-secondary">{$t("payments.assetToSend")}</div>
      <div class="flex items-center gap-2">
        <span style="color:#26A17B">₮</span> {currentUsdtAmount.toLocaleString(locale, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
      </div>
    </div>

    <div>
      <div class="text-lg text-secondary">{$t("payments.amount")}</div>
      <div class="flex items-center gap-2">
        <iconify-icon noobserver icon="ph:lightning-fill" class="text-yellow-300" width="24"></iconify-icon>
        {s(currentSats, locale)}
        <span class="text-secondary text-lg">{f((currentSats / sats) * fiatRate, currency, locale)}</span>
      </div>
    </div>

    <div>
      <div class="text-lg text-secondary">{$t("payments.platformFee")}</div>
      <div class="flex items-center gap-2">
        <iconify-icon noobserver icon="ph:lightning-fill" class="text-yellow-300" width="24"></iconify-icon>
        {s(currentFee, locale)}
        <span class="text-secondary text-lg">{f((currentFee / sats) * fiatRate, currency, locale)}</span>
      </div>
    </div>
    ...
    <div class="border-t border-base-200 pt-4">
      ...
      <div class="flex items-center gap-2">
        <iconify-icon noobserver icon="ph:lightning-fill" class="text-yellow-300" width="24"></iconify-icon>
        {s(currentSats + currentFee, locale)}
        <span class="text-secondary text-lg">{f(((currentSats + currentFee) / sats) * fiatRate, currency, locale)}</span>
      </div>
    </div>
  </div>
